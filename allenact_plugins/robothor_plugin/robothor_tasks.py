...
class ObjectGestureNavTask(Task[RoboThorEnvironment]):
    _actions = (MOVE_AHEAD, ROTATE_LEFT, ROTATE_RIGHT, END, LOOK_UP, LOOK_DOWN)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._all_metadata_available:
            ...
            # ) # TODO: WHY THIS IS DIFFERENT FROM THE DATA ON FILE?
            self.last_geodesic_distance = self.task_info["distance_to_target"]
            self.optimal_distance = self.last_geodesic_distance
            self.closest_geo_distance = self.last_geodesic_distance
    ...
    def shaping(self) -> float:
        ...
        geodesic_distance = self.env.distance_to_point(
            self.task_info["target_position"]
        )
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        metrics = super(ObjectGestureNavTask, self).metrics()
        if self._all_metadata_available:
            dist2tget = self.env.distance_to_point(self.task_info["target_position"])
            ...
            metrics = {
                **metrics,
                "success": self._success,
                "1stop_success": 0 if self.num_stops > 1 else self._success,
                "2stop_success": 0 if self.num_stops > 2 else self._success,
                "3stop_success": 0 if self.num_stops > 3 else self._success,
                "total_reward": np.sum(self._rewards),
                "dist_to_target": dist2tget,
                "spl": 0 if spl is None else spl,
                "1stop_spl": 0 if spl is None or self.num_stops > 1 else spl,
                "2stop_spl": 0 if spl is None or self.num_stops > 2 else spl,
                "3stop_spl": 0 if spl is None or self.num_stops > 3 else spl,
